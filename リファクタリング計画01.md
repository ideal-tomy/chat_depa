...
| ✅ | タスク | コマンド／備考 |
|----|--------|----------------|
| [ ] | **0-1. ブランチ作成** | `git checkout -b refactor/phase1-core-cleanup` |
| [ ] | **0-2. ESLint & Prettier を強制** | `pnpm add -D eslint prettier eslint-config-next` → `.eslintrc` を strict に |
...
- [ ] 全 app-route handler に `export const dynamic = 'force-dynamic'` を宣言
- [ ] `api-client.ts` を **相対パス fetch 専用**に修正（CORS を根絶）

### 3-B. ポイント / プロフィール API 改修

- [ ] 未ログイン時は HTTP 200 + デフォルト値を返却
...
- [ ] `next.config.js` の `ignoreBuildErrors` を削除
